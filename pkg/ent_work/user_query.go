...
// QueryCreations chains the current query on the "creations" edge.
func (uq *UserQuery) QueryCreations() *CreationQuery {
	query := (&CreationClient{config: uq.config}).Query()
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withCreations: uq.withCreations.Clone(),
...
// WithCreations tells the query-builder to eager-load the nodes that are connected to
// the "creations" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithCreations(opts ...func(*CreationQuery)) *UserQuery {
	query := (&CreationClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withCreations = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withCreations; query != nil {
		if err := uq.loadCreations(ctx, query, nodes,
			func(n *User) { n.Edges.Creations = []*Creation{} },
			func(n *User, e *Creation) { n.Edges.Creations = append(n.Edges.Creations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadCreations(ctx context.Context, query *CreationQuery, nodes []*User, init func(*User), assign func(*User, *Creation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(creation.FieldUserID)
	}
	query.Where(predicate.Creation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.CreationsColumn), fks...))
	}))
